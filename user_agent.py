...
class TestUserAgent:
    ...
    @pytest.mark.parametrize("ua_string, expected_value", ua_arr)
    def test_user_agent(self, ua_string, expected_value):
        url = "https://playground.learnqa.ru/ajax/api/user_agent_check"

        response = requests.get(url, headers={"User-Agent": ua_string})
        response_dict = response.json()

        keys = ["device", "browser", "platform"]
        for key in keys:
            assert key in response_dict, f"User Agent '{ua_string}' has not '{key}' in the response"
            assert response_dict[key] == expected_value[key], f"User Agent '{ua_string}' have incorrect '{key}'"
